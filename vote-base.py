...
def main():
    ...
    # -------------------------------
    # 1) Conectar à rede Base via RPC
    # -------------------------------
    ...
    if not web3.is_connected():
        print("❌ Falha ao conectar na rede Base via RPC.")
        sys.exit(1)
    ...
    # Configuração de gas: Limite fixo
    GAS_LIMIT = 100000  # Gas limite reduzido
    ...
    def vote():
        try:
            ...
            # Obtendo dinamicamente o gas price e pausando se for maior que 0.05 gwei
            GAS_PRICE_THRESHOLD = 0.05
            gas_price = web3.eth.gas_price
            gas_price_gwei = float(web3.from_wei(gas_price, 'gwei'))
            while gas_price_gwei > GAS_PRICE_THRESHOLD:
                print(f"⏳ Gas price atual: {gas_price_gwei:.6f} gwei, que é maior que o limite de {GAS_PRICE_THRESHOLD} gwei.")
                print("⏳ Pausando 10 segundos antes de reavaliar o gas price...")
                time.sleep(10)
                gas_price = web3.eth.gas_price
                gas_price_gwei = float(web3.from_wei(gas_price, 'gwei'))
            print(f"✅ Gas price aceitável: {gas_price_gwei:.6f} gwei.")
            ...
            print(f"   → Gas Price: {gas_price_gwei:.6f} gwei | Gas Limit: {GAS_LIMIT}")
